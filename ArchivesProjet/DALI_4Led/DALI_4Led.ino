      ...
      while (client.connected() && currentTime - previousTime <= timeoutTime) { // loop while the client's connected
        ...
        if (client.available()) {             // if there's bytes to read from the client,
          ...
          if (c == '\n') {                    // if the byte is a newline character
            ...
            if (header.indexOf("GET /s00/on") >= 0) {//-----------------s00
              String s ="s00";
              outputs00 = "on";
              int shortAddress = s.substring(1, 3).toInt();
              ShortOnOff(shortAddress, ON); 
            } else if (header.indexOf("GET /s00/off") >= 0) {
              String s ="s00";
              outputs00 = "off";
              int shortAddress = s.substring(1, 3).toInt();
              ShortOnOff(shortAddress, OFF); 
            }
            if (header.indexOf("GET /s01/on") >= 0) {//-----------------s01
              String s ="s01";
              outputs01 = "on";
              int shortAddress = s.substring(1, 3).toInt();
              ShortOnOff(shortAddress, ON); 
            } else if (header.indexOf("GET /s01/off") >= 0) {
              String s ="s01";
              outputs01 = "off";
              int shortAddress = s.substring(1, 3).toInt();
              ShortOnOff(shortAddress, OFF); 
            }
            if (header.indexOf("GET /s02/on") >= 0) {//-----------------s02
              String s ="s02";
              outputs02 = "on";
              int shortAddress = s.substring(1, 3).toInt();
              ShortOnOff(shortAddress, ON); 
            } else if (header.indexOf("GET /s02/off") >= 0) {
              String s ="s02";
              outputs02 = "off";
              int shortAddress = s.substring(1, 3).toInt();
              ShortOnOff(shortAddress, OFF); 
            }
            if (header.indexOf("GET /s03/on") >= 0) {//-----------------s03
              String s ="s03";
              outputs03 = "on";
              int shortAddress = s.substring(1, 3).toInt();
              ShortOnOff(shortAddress, ON); 
            } else if (header.indexOf("GET /s03/off") >= 0) {
              String s ="s03";
              outputs03 = "off";
              int shortAddress = s.substring(1, 3).toInt();
              ShortOnOff(shortAddress, OFF); 
            ...
            message.trim();
              ...
              if (message.startsWith("SliderValue:")) {
                  String valueString = message.substring(12);
                  sliderPowerOnValue = valueString.toInt();
                  Serial.print("Slider value: ");
                  Serial.println(sliderPowerOnValue);
                  SetPowerOnLevel(sliderPowerOnValue);
                  }
               }
            }
            ...
            else if (header.indexOf("GET /smallDay") >= 0) {
            ...
            //state equalization
              if (outputs00=="off" && outputs01=="off" && outputs02=="off" && outputs03=="off"){
                outputBroadCastState = "off";
              } else if (outputs00=="on" && outputs01=="on" && outputs02=="on" && outputs03=="on"){
                outputBroadCastState = "on";
              }
                  ...
                  client.println("<h2>");
                    ...
                    client.println("<d1>");
                        ...
                        client.println("<e1>");
                        	client.println("<c2>");
                              ...
                              client.println("<p><input type=\"range\" onchange=\"updateSliderPWM(this)\" id=\"pwmSlider\" min=\"0\" max=\"254\" value=\"select a value\" step=\"1\" class=\"slider\"></p>");
                              ...
                              //client.print("function sendSliderValue() {var sliderValue = document.getElementById("pwmSlider").value;");
                              //client.print("var xhr = new XMLHttpRequest();xhr.open("GET", \"/slider?value=" + sliderValue, true);xhr.send();");
                              //client.print("var message = \"SliderValue:\" + sliderValue;Serial.println(message);}");
                              client.println("</script>");
                            
                              //PowerOn Button
                                client.println("<a href=\"/powerOn\" onclick=\"sendSliderValue()\"><button class=\"button\">Set Value</button></a>");
                    ...
                    client.println("<subtitle>Basics</subtitle>");
                        client.println("<c1>");
                            ...
                            if (outputBroadCastState=="off") {
                              client.println("<a href=\"/broadcast/on\"><button class=\"button\">ON</button></a>");
                            } else {
                              client.println("<a href=\"/broadcast/off\"><button class=\"button button2\">OFF</button></a>");
                            } 
                			    client.println("<txt>LED 1</txt>");
                            if (outputs00=="off") {
                              client.println("<a href=\"/s00/on\"><button class=\"button\">ON</button></a>");
                            } else {
                              client.println("<a href=\"/s00/off\"><button class=\"button button2\">OFF</button></a>");
                            } 
                			    client.println("<txt>LED 2</txt>");
                            if (outputs01=="off") {
                              client.println("<a href=\"/s01/on\"><button class=\"button\">ON</button></a>");
                            } else {
                              client.println("<a href=\"/s01/off\"><button class=\"button button2\">OFF</button></a>");
                            }
                          client.println("<txt>LED 3</txt>");
                            if (outputs02=="off") {
                              client.println("<a href=\"/s02/on\"><button class=\"button\">ON</button></a>");
                            } else {
                              client.println("<a href=\"/s02/off\"><button class=\"button button2\">OFF</button></a>");
                            }
                          client.println("<txt>LED 4</txt>");
                            if (outputs03=="off") {
                              client.println("<a href=\"/s03/on\"><button class=\"button\">ON</button></a>");
                            } else {
                              client.println("<a href=\"/s03/off\"><button class=\"button button2\">OFF</button></a>");
                            }  
                			  client.println("</c1>");
                	  client.println("</d1>");
                    ...
                    client.println("<d1>");
                      client.println("<subtitle>Special Mods</subtitle>");
                        ...
                        client.println("<c1>");
                				  client.println("<txt>Small Day simulation</txt>");
                            //Small day simulation Button     
                            client.println("<a href=\"/smallDay\"><button class=\"button\">Wake Up</button></a>");
                			  client.println("</c1>");
...
void GradientTimeScale(int timeScale) {
  int a=7;
  int i=timeScale;
  bool state = true;
  if (state){
    int t = timeClient.getSeconds();
    ...
    SetMaxLevel(i);
    delayMilisec(DALI_TWO_PACKET_DELAY);
    DaliTransmitCMD(BROADCAST_C, ON_C);//--------------------là mec il faut que je permette à la condition bol de changer pour sortir des boucles et clic boutonoff
    delayMilisec(DALI_TWO_PACKET_DELAY);
    if(t > 30){
      state = false;
    }

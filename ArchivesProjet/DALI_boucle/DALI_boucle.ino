...
{    
    ...
    //Hour and date
    timeUTC.update();
    String fullHour = timeUTC.getFormattedTime();
    int hour = timeUTC.getHours();
    int minute = timeUTC.getMinutes();
    int second = timeUTC.getSeconds();
      ...
      while (client.connected() && currentTime - previousTime <= timeoutTime) { // loop while the client's connected
        ...
        if (client.available()) {             // if there's bytes to read from the client,
          ...
          if (c == '\n') {                    // if the byte is a newline character
            ...
            if (header.indexOf("GET /simulation") >= 0) {
              while (1){
                int second = timeUTC.getSeconds();
                Simulation(second);
              }
            ...
            if (header.indexOf("GET /testhour") >= 0) {
              int hour2 = timeUTC.getHours();
              Serial.print(hour2);
              Serial.println(" Heure UTC");
              int hFR= hour2+2;
              Serial.print(hFR);
              Serial.println(" Heure UTC+2");
            }
                  ...
                  client.println("<h2>");
                    ...
                    client.println("<d1>");
                      client.println("<subtitle>Special Mods</subtitle>");
                        client.println("<c1>");
                				  client.println("<txt>Test Hour</txt>");
                            //Gradientslow Button     
                            client.println("<a href=\"/testhour\"><button class=\"button\">TEST</button></a>");
                			  client.println("</c1>");
                        client.println("<c1>");
                        ...
                        client.println("<c1>");
                				  client.println("<txt>Simulation</txt>");
                            //Gradient minute Button
                            client.println("<a href=\"/simulation\"><button class=\"button\">Simulation</button></a>");
                			  client.println("</c1>");
                        client.println("<c1>");
                				  client.println("<txt>Small Day</txt>");

...
public:
  ...
  void LongPressed(){ //If (21:45<hour<23:45) --> Add 15min before SetNight

    int hour = (utc_delay + timeClient.getHours())%24;
        int minute = timeClient.getMinutes();
        Serial.println(hour);
        Serial.println(minute);
        if((hour<21) || (hour==21 && minute<45)){Serial.println(" Too early ! ");}
        else if(hour==23&&minute>45){Serial.println(" Too late ! ");}
        else{//add 15 min
...
void loop() {
  ...
  else if(dayRunning==false){BroadcastOnOff(OFF);}
...
void DayCycle() {
    ...
    int currentHour = (utc_delay + timeClient.getHours())%24;
    ...
    //Serial.print("sunElevation : ");Serial.println(sunElevation);

    if(currentHour>=6&&currentHour<18)//-----------------------------------------06:00 / 18:00
      {
    ...
    if((currentHour==21&&currentMinute>=45)||(currentHour==22)||(currentHour==23&&currentMinute<45))//-----21:45 / 23:45
      {
    ...
    if(currentHour==23&&currentMinute>=45)//------------------------------------------------23:45 / 00:00
      {
        if(nightState==false)//-----already in night ?
        {
            minuteDelay++;
